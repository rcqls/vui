...
fn window_draw(win &ui.Window) {
	state := &State(win.state)
	if state.show_imgui {
		C.igShowDemoWindow(&state.show_imgui)
	}
} 
...
fn btn_imgui_click(mut a State, b &ui.Button) {
	a.show_imgui = !a.show_imgui
}
